...
function WheelOfDestiny.getSliceIndex(position)
    ...
    for _, index in pairs(wheelClick) do
        if WheelButtons[index] then
            ...
            if radius == BIG_LARGE_CIRCLE and
                (largeCircle:inArea(position) or bigMediumCircle:inArea(position) or mediumCircle:inArea(position) or
                    smallCircle:inArea(position)) then
                shouldProcess = false
            elseif radius == BIG_LARGE_CIRCLE then
                circle = bigLargeCircle
            elseif radius == LARGE_CIRCLE and
                (bigMediumCircle:inArea(position) or mediumCircle:inArea(position) or smallCircle:inArea(position)) then
                shouldProcess = false
            ...
            if shouldProcess and circle:inArea(position) then
...
function WheelOfDestiny.insertPoint(index, points)
    ...
    if points > 0 then
        ...
        if points >= bonus.maxPoints then
            ...
            if maxcolor > 0 then
                wheelPanel:recursiveGetChildById('colorWheel_' .. index):setImageSource(button.colorImageBase ..
                                                                                            maxcolor)
                wheelPanel:recursiveGetChildById('colorWheel_' .. index):setOpacity(0.6)
            end
        end
...
function WheelOfDestiny.onDestinyWheel(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested,
    usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
    if not table.isIn({1, 2, 3, 4, 5}, vocationId) then
        ...
        if not openWheel then
            openWheel = displayGeneralBox(tr('Info'), tr(
                "To be able to use the Wheel of Destiny, a character must be at leat level 51, be promoted and have active\nPremium Time."),
                {{
                    text = tr('Ok'),
                    callback = cancelFunc
                }}, cancelFunc)
    ...
    WheelOfDestiny.create(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested,
        usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
...
function onAddMax(index)
    ...
    wheelOfDestinyWindow.selection.tabContent.dedicationPb:setValue(WheelOfDestiny.pointInvested[index], 0,
        bonus.maxPoints)
    wheelOfDestinyWindow.selection.tabContent.dedicationPb:setText(
        WheelOfDestiny.pointInvested[index] .. " / " .. bonus.maxPoints)
...
function WheelOfDestiny.configureDedicationPerk()
    ...
    for id, bonus in pairs(WheelBonus) do
        local index = id + 1
        if WheelOfDestiny.isLit(index) then
...
function WheelOfDestiny.configureSummary()
    ...
    for _, t in ipairs(f_table) do
        ...
        elseif t == "Life Leech" then
            ...
            if lifeleech and lifeleech.points ~= 0 then
        ...
        elseif t == "Mana Leech" then
            ...
            if manaleech and manaleech.points ~= 0 then
...
function WheelOfDestiny.create(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested,
    usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
    ...
    -- order
    local orderned = {15, 9, 14, 3, 8, 13, 2, 7, 1, 16, 10, 17, 4, 11, 18, 5, 12, 6, 22, 23, 28, 24, 29, 34, 30, 35, 36,
                      21, 20, 27, 19, 26, 33, 25, 32, 31}
    ...
    local totalPoints = WheelOfDestiny.points + (WheelOfDestiny.extraGemPoints + WheelOfDestiny.scrollPoints)
    wheelOfDestinyWindow.selection.points:setText("[7]" .. totalPoints - WheelOfDestiny.usedPoints .. " / " ..
                                                      totalPoints)
...
function WheelOfDestiny.onWheelPassiveClick(domain)
    ...
    wheelOfDestinyWindow.selection.tabContent.information:setTooltip(
...
function WheelOfDestiny.showNewPreset(createPreset)
    ...
    newPresetWindow.contentPanel.copyPreset:setText(string.format("Copy preset '%s'",
        WheelOfDestiny.currentPreset.presetName))
end
...
function WheelOfDestiny.onImportConfig(base64Data)
    ...
    local importMap = { -- TL (Green) - (Top -> Bot)
    15, 14, 9, 13, 8, 3, 7, 2, 1, -- TR (Red) - (Right -> Top)
    16, 17, 10, 18, 11, 4, 12, 5, 6, -- BR (Purple) - (Right -> Bot)
    22, 23, 28, 24, 29, 34, 30, 35, 36, -- BL (Blue) - (Left -> Bot)
...
function WheelOfDestiny.onExportPreset()
    ...
    exportCodeWindow = displayGeneralBox('Copy to Clipboard',
        tr("Copy export code or URL of the planner to clipboard."), {{
            text = tr('Code'),
            callback = codeButton
        }, {
            text = tr('URL'),
...
function WheelOfDestiny.getExportCode(preset)
    ...
    -- Export map: converts internal position to export position
    -- This is the INVERSE of importMap used in onImportConfig
    local exportMap = { -- TL (Green) - (Top -> Bot)
    15, 14, 9, 13, 8, 3, 7, 2, 1, -- TR (Red) - (Right -> Top)
    16, 17, 10, 18, 11, 4, 12, 5, 6, -- BR (Purple) - (Right -> Bot)
    22, 23, 28, 24, 29, 34, 30, 35, 36, -- BL (Blue) - (Left -> Bot)
    21, 20, 27, 19, 26, 33, 32, 25, 31}

    -- Convert internal pointInvested array to export order
    local exportPoints = {}
    for i = 1, 36 do
        exportPoints[i] = 0
    end
    ...
    -- Pack the points in export order
...
function WheelOfDestiny.onConfirmCreatePreset()
    ...
    WheelOfDestiny.create(WheelOfDestiny.playerId, WheelOfDestiny.canView, WheelOfDestiny.changeState,
        WheelOfDestiny.vocationId, basePoints, WheelOfDestiny.scrollPoints, dataCopy.pointInvested,
        WheelOfDestiny.usedPromotionScrolls, dataCopy.equipedGems, currentAtelierGems, currentBasicMods,
        currentSupremeMods)
...
function WheelOfDestiny.onDeletePreset()
    ...
    local message = string.format("Do you really  want to delete the preset '%s'?",
        WheelOfDestiny.currentPreset.presetName)
...
function WheelOfDestiny.configurePresets()
    ...
    local status, err = pcall(function()
        ...
        presetPanel.onChildFocusChange = function(self, selection, oldSelection)
...
function WheelOfDestiny.onPreparePresetClick(list, selection, oldSelection)
    ...
    local message = string.format(
        "You have not saved the changes you made to preset '%s'.\nDo you want to save your current changes and active the preset?",
        WheelOfDestiny.currentPreset.presetName)
...
function WheelOfDestiny.onPresetClick(list, selection, oldSelection)
    ...
    -- Cache presetData locally because configurePresets() will destroy this widget
    ...
    if player and not player:hasState(PlayerStates.Pz) then
        local managePresetsButton =
            wheelWindow.mainPanel.wheelMenu.info.presetTabBar:getChildById('managePresetsButton')
        toggleTabBarButtons('informationButton')
    ...
    WheelOfDestiny.create(WheelOfDestiny.playerId, WheelOfDestiny.canView, WheelOfDestiny.changeState,
        WheelOfDestiny.vocationId, pointsInvested, WheelOfDestiny.scrollPoints, presetData.pointInvested,
        WheelOfDestiny.usedPromotionScrolls, presetData.equipedGems, WheelOfDestiny.atelierGems,
        WheelOfDestiny.basicModsUpgrade, WheelOfDestiny.supremeModsUpgrade)
...
function WheelOfDestiny.determinateCurrentPreset()
    ...
    -- If no matching preset found, initialize to first preset
    if not WheelOfDestiny.currentPreset or table.empty(WheelOfDestiny.currentPreset) then
        if not table.empty(WheelOfDestiny.internalPreset) then
            WheelOfDestiny.currentPreset = WheelOfDestiny.internalPreset[1]
            local presetLabel = wheelWindow:recursiveGetChildById("presetLabel")
            if presetLabel then
...
function WheelOfDestiny.generateInternalPreset()
    ...
    for k, v in pairs(WheelOfDestiny.externalPreset.presets) do
        ...
        -- Invalid data
        if not table.empty(data) and vocationString == getVocationSt(playerVocation) then
...
function WheelOfDestiny.configureEquippedGems()
    for i = 0, 3 do
        ...
        local backGroundImage = (filledCount == 0 and "backdrop_skillwheel_largebonus_socketdisabled_" .. i or
                                    "backdrop_skillwheel_largebonus_socketenabled_" .. i)
...
function WheelOfDestiny.onGemVesselClick(domain)
    ...
    wheelOfDestinyWindow.selection.gemContent.sealedInfo:setText(
        tr("%s Vessel (VR %s)", vesselStatus, (filledCount == 0 and "0" or romanLetter[filledCount])))
    ...
    if data then
        ...
        local text = {}
        setStringColor(text, tr("+%s Damage and Healing", (data.gemType == 2 and 2 or 1)),
            (filledCount == 3 and "#c0c0c0" or "#707070"))
...
function WheelOfDestiny.onChangeGemButton(domain)
    ...
    local currentDomain = WheelOfDestiny.lastSelectedGemVessel and
                              WheelOfDestiny.lastSelectedGemVessel:getId():gsub("selectVessel", "") or 0
...
function WheelOfDestiny.setupPointsTooltip()
    local tooltip = tr(WheelPointTooltip, WheelOfDestiny.levelPoints, WheelOfDestiny.extraGemPoints,
        WheelOfDestiny.scrollPoints)
