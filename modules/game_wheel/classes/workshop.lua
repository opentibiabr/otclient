...
function Workshop.getDataByBonus(bonusID, supreme)
    for _, data in pairs(fragmentList) do
        if (supreme and data.supreme and bonusID == data.modID) or
            (not supreme and not data.supreme and bonusID == data.modID) then
            return data
        end
    end
    return nil
...
function Workshop.createFragments()
    ...
    for id = 0, #FlatSupremeMods do
        ...
        if info then
            if not (id == 4 and vocationId > 6) then
                info.modID = id
                info.supreme = true
...
function Workshop.showFragmentList(startUp, nextPage, selectCurrent, searchText, focusIndex)
    ...
    if not startUp and not focusIndex then
        currentWorkshopPage = nextPage and math.min(maxPages, currentWorkshopPage + 1) or
                                  math.max(1, currentWorkshopPage - 1)
    end
    ...
    for i, widget in ipairs(fragmentPanel:getChildren()) do
        ...
        if k <= endList then
            ...
            if info then
                ...
                local count = isSupreme and (WheelOfDestiny.supremeModCount[tostring(info.modID)] or 0) or
                                  (WheelOfDestiny.basicModCount[tostring(info.modID)] or 0)
                updateWidget(widget, info, isSupreme and equippedSupreme or equippedBasic, count)
                g_logger.debug(string.format("[WorkshopCount] modID=%d supreme=%s -> count=%d (key='%s')", info.modID,
                    tostring(isSupreme), count, tostring(info.modID)))
            end
        end
    end
...
function Workshop.onSelectChild(list, selected)
    ...
    local resourceCheck = isSupreme and player:getResourceBalance(ResourceTypes.GREATER_FRAGMENTS) or
                              player:getResourceBalance(ResourceTypes.LESSER_FRAGMENTS)
    ...
    if resourceCheck < fragmentCost then
        if not string.empty(blockedTooltip) then
            blockedTooltip = tr("%s\nYou need at least %s greater fragments to enhance mods of this quality.",
                blockedTooltip, fragmentCost)
        else
...
function Workshop.onUpgradeModification(button)
    ...
    g_logger.debug(string.format(
        "[Workshop] Requesting UpgradeModification -> action=4 | fragmentType=%d | pos=%d | supreme=%s", fragmentType,
        pos, tostring(supreme)))
...
function Workshop.getBonusDescription(modInfo, relativeTier)
    ...
    local targetTier = modInfo.supreme and WheelOfDestiny.supremeModsUpgrade[modInfo.modID] or
                           WheelOfDestiny.basicModsUpgrade[modInfo.modID]
...
function Workshop.getUpgradeBonus(baseBonus, modID, supreme, targetTier)
    local modTier = targetTier and targetTier or
                        (supreme and WheelOfDestiny.supremeModsUpgrade[modID] or WheelOfDestiny.basicModsUpgrade[modID])
    if not modTier then
        return baseBonus
    end
...
function Workshop.getSortList(sortOption, equippedBasic, equippedSupreme, text)
    ...
    for _, data in pairs(fragmentList) do
        ...
        if not (text and not string.empty(text) and not matchText(text, data.tooltip)) then
