...
function GemAtelier.redirectToGem(gemData)
    ...
    for i, data in pairs(WheelOfDestiny.atelierGems) do
        if not ((sortQuality > 1 and data.gemType ~= sortQuality - 2) or
            (sortAffinity > 1 and data.gemDomain ~= sortAffinity - 2)) then
    ...
    gemList.onChildFocusChange = function(self, selected)
...
function GemAtelier.showGems(selectFirst, lastIndex)
    ...
    for i, data in pairs(WheelOfDestiny.atelierGems) do
        if not data.gemID or data.gemID < 0 then
            ...
            if not (lockedOnly and not isLocked) and
               not (sortQuality > 1 and data.gemType ~= sortQuality - 2) and
               not (sortAffinity > 1 and data.gemDomain ~= sortAffinity - 2) and
               not (#currentSearchText > 0 and not GemAtelier.matchGemText(data)) then
                table.insert(totalGemList, data)
            end
        end
    end
    ...
    if #children == 0 then
        ...
        elseif lastSelectedGem and lastSelectedGem:isVisible() and lastSelectedGem.gemID then
            ...
            if targetIndex > 0 then
                ...
                g_logger.debug(string.format("[GemAtelier] gemID %d not found among displayed children.",
                    lastSelectedGem.gemID or -1))
                if #children > 0 then
...
function GemAtelier.setupGemWidget(widget, data)
    ...
    if data and data.gemID and data.gemID >= 0 then
        ...
        widget.gemID = -1
        g_logger.debug(string.format("[GemAtelier] gem without valid gemID — gemID=%s (type=%s)",
            tostring(data and data.gemID), type(data and data.gemID)))
        return false
    end
    ...
    if not data.gemType or not data.gemDomain then
        g_logger.debug(string.format("[GemAtelier] incomplete gem data: id=%s type=%s domain=%s", tostring(data.gemID),
            tostring(data.gemType), tostring(data.gemDomain)))
        return false
    end
    ...
    if not tmpData then
        g_logger.debug(string.format("[GemAtelier] gem id %d not found in GemVocations[%d][%d]", data.gemID or -1,
            WheelOfDestiny.vocationId or -1, data.gemType or -1))
        return false
    end
    ...
    g_logger.debug(string.format("[GemAtelier] Locker configured for gemID=%d | locked=%d | checked=%s | visible=%s",
        data.gemID or -1, data.locked or -1, tostring(widget.locker:isChecked()), tostring(widget.locker:isVisible())))
    ...
    if not gemTypeWidget then
        g_logger.debug(string.format("[GemAtelier] gemTypeWidget modType%d not found for gemID=%d", data.gemType,
            data.gemID))
        return false
    end
    ...
    GemAtelier.setupGemSlot(gemTypeWidget.fragmentType0.gemMod0, data.lesserBonus, WheelOfDestiny.basicModsUpgrade,
        false, data, 0)
    GemAtelier.setGemUpgradeImage(gemTypeWidget.fragmentType0, data.lesserBonus, WheelOfDestiny.basicModsUpgrade, nil)

    if data.gemType > 0 then
        GemAtelier.setupGemSlot(gemTypeWidget.fragmentType1.gemMod1, data.regularBonus, WheelOfDestiny.basicModsUpgrade,
            false, data, 1)
        GemAtelier.setGemUpgradeImage(gemTypeWidget.fragmentType1, data.regularBonus, WheelOfDestiny.basicModsUpgrade,
            WheelOfDestiny.basicModsUpgrade[data.lesserBonus] or 0, true)
    end

    if data.gemType > 1 then
        GemAtelier.setupGemSlot(gemTypeWidget.fragmentType2.gemMod2, data.supremeBonus,
            WheelOfDestiny.supremeModsUpgrade, true, data, 2)
        local effectiveBonus = math.min(WheelOfDestiny.basicModsUpgrade[data.lesserBonus] or 0,
            WheelOfDestiny.basicModsUpgrade[data.regularBonus] or 0)
        GemAtelier.setGemUpgradeImage(gemTypeWidget.fragmentType2, data.supremeBonus, WheelOfDestiny.supremeModsUpgrade,
            effectiveBonus)
    end
...
function GemAtelier.setupGemSlot(gemSlot, bonus, upgradeData, isSupreme, gemData, gemPosition)
    gemSlot:setImageClip(bonus * (isSupreme and 35 or 30) .. " 0 " .. (isSupreme and 35 or 30) .. " " ..
                             (isSupreme and 35 or 30))
...
function GemAtelier.onSelectGem(selected, clicked)
    ...
    g_logger.debug(string.format("[GemAtelier] onSelectGem: gemID=%d currentGemList size=%d", selected.gemID,
        #currentGemList))
    ...
    if not gemData then
        g_logger.debug(string.format("[GemAtelier] gemData is nil for gemID=%s (currentGemList size=%d)",
    ...
    panel.clickedContent.placeVessel.onClick = function()
        GemAtelier.manageVessel(false)
    end
    panel.clickedContent.removeVessel.onClick = function()
    ...
    g_logger.debug(string.format("[GemAtelier] Buttons updated -> changeState=%d locked=%d canInteract=%s",
    ...
    if gemCount < 2 then
        ...
        destroyTip = tr("%s%sYou cannot destroy the last gem of the domain.", destroyTip,
            (#destroyTip > 0 and "\n" or ""))
    end
    if totalBalance < price then
        switchTip = tr("%s%sYou need at least %s gold to change the domain of this gem.", switchTip,
            (#switchTip > 0 and "\n" or ""), comma_value(price))
    end
    if gemData.locked == 1 then
        switchTip = tr("%s%sBefore you can change the domain of this gem, you must unlock it.", switchTip,
            (#switchTip > 0 and "\n" or ""))
        destroyTip = tr("%s%sBefore you can destroy the gem, you must unlock it.", destroyTip,
            (#destroyTip > 0 and "\n" or ""))
    end
    if GemAtelier.isGemEquipped(gemData.gemID) then
        switchTip = tr("%s%sYou must remove the gem from its vessel before you can switch its domain.", switchTip,
            (#switchTip > 0 and "\n" or ""))
        destroyTip = tr("%s%sThe gem must be removed from its vessel before it can be destroyed.", destroyTip,
            (#destroyTip > 0 and "\n" or ""))
    end
    ...
    if totalBalance >= price then
        panel.clickedContent.switch:setTooltip(tr(
            "%s%sSwitch the gem\'s domain one step clockwise by paying the free of %s gold.", switchTip,
            (#switchTip > 0 and "\n" or ""), comma_value(price)))
    end
end
...
function GemAtelier.setupModAvailable(widget, gemType, vesselCount, gemData)
    ...
    local gemBonusID =
        (gemType == 0 and gemData.lesserBonus or gemType == 1 and gemData.regularBonus or gemType == 2 and
            gemData.supremeBonus)
...
function GemAtelier.manageVessel(remove)
    ...
    g_logger.debug(string.format("[GemAtelier] manageVessel -> %s gemID=%d domain=%d | equipedGems={%s}",
        remove and "remove" or "equip", gemData.gemID, gemData.gemDomain, table.concat(WheelOfDestiny.equipedGems, ", ")))
...
function GemAtelier.getGemDataById(id)
    if type(id) ~= "number" or id < 0 then
        g_logger.debug(string.format("[GemAtelier] getGemDataById called with invalid id: %s (type=%s)", tostring(id),
            type(id)))
        return nil
    end
    for _, data in pairs(WheelOfDestiny.atelierGems) do
    ...
    g_logger.debug(string.format("[GemAtelier] getGemDataById: gemID=%d not found in atelierGems (total gems=%d)", id,
...
function sendgemAction(actionType, param, pos)
    ...
    if actionType == 3 then
        scheduleEvent(function()
            ...
            g_logger.debug(string.format("[GemAtelier] Toggled local lock of gem id=%d -> %s", param,
                gem.locked == 1 and "locked" or "unlocked"))
...
function GemAtelier.onSwitchDomain(button)
    ...
    if gemData then
        ...
        g_logger.debug(string.format("[GemAtelier] onSwitchDomain: gemData not found for gemID=%s",
            tostring(lastSelectedGem.gemID)))
    end
end
...
function GemAtelier.setupVesselPanel()
    ...
    for i = 0, 3 do
        ...
        if not background or not gemContainer or not gemItem then
            g_logger.debug(string.format("[GemAtelier] Estrutura do slot %d incompleta (bg=%s, container=%s, gem=%s)",
        ...
        if type(id) == "number" and id >= 0 then
            ...
            if data then
                ...
                if background and gemContainer and gemItem then
                    ...
                    g_logger.debug(string.format("[GemAtelier] Slot %d incomplete when applying gem %d.",
...
function GemAtelier.onClickVessel(widget, domain)
    ...
    if gemData and gemData.gemDomain == domain then
        g_logger.debug(string.format("[DebugClick] Gem found id=%d domain=%d (valid for vessel)", gemData.gemID,
            gemData.gemDomain))
    ...
    if fallbackGem then
        g_logger.debug(string.format("[DebugClick] No vessel equipped -> focusing smallest gem of domain %d (gemID=%d)",
            domain, fallbackGem.gemID))
